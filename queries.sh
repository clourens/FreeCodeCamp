...
echo "$($PSQL "(SELECT name FROM games INNER JOIN teams ON winner_id=team_id  WHERE round='Eighth-Final' AND year=2014) UNION (SELECT name FROM games INNER JOIN teams ON opponent_id=team_id  WHERE round='Eighth-Final' AND year=2014) ")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM games INNER JOIN teams ON winner_id=team_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT DISTINCT year,name FROM games INNER JOIN teams ON winner_id=team_id WHERE round='Final' ORDER BY year, name")"
